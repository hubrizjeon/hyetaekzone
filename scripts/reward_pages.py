...
(function(){
  ...
  var h=location.hash, tok=h.match(/login=([A-Za-z0-9_-]{20,})/), rw=h.match(/rw=([a-z0-9-]+)/);
  ...
  if(rw){ var kc=rw[1].match(/-(koe\d+)$/), base=kc?rw[1].slice(0,-kc[0].length):rw[1];
    if(MSG[base]) note(MSG[base]+(kc?' (카카오 오류 '+kc[1].toUpperCase()+')':'')); }
